...
This project depends on [FrankenTUI](../frankentui) via local path dependencies in `Cargo.toml`. The `../frankentui` directory must be present to build.
...
The release binary is named `lists` (see `[[bin]]` in `Cargo.toml`).
...
- Use inline PR links: `([#123](https://github.com/kcosr/assistant-cli/pull/123))`

### Attribution

- Internal changes: `Fixed foo bar ([#123](https://github.com/kcosr/assistant-cli/pull/123))`
- External contributions: `Added feature X ([#456](https://github.com/kcosr/assistant-cli/pull/456) by [@user](https://github.com/user))`
...
3. Run the release script:
   ```bash
   node scripts/release.mjs patch    # Bug fixes (0.0.1 -> 0.0.2)
   node scripts/release.mjs minor    # New features (0.0.2 -> 0.1.0)
